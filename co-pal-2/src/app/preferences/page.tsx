    ...
    <div className="min-h-screen bg-background pb-16 flex items-start">
      <div className="max-w-sm w-full mx-auto px-4 pt-8">
        ...
        <p className="text-sm text-muted-foreground mb-5">
          Set what you&apos;re looking for in a partner to improve matching.
        </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            ...
            <div className="flex gap-2 mt-2">
              {(["male", "female"] as const).map((g) => (
                <button
                  ...
                  onClick={() => {
                    const current =
                      (watch("preferredGenders") as (typeof g)[]) || [];
                    const next = current.includes(g)
                      ? current.filter((x: any) => x !== g)
                      : [...current, g];
                    setValue("preferredGenders", next as any);
                  }}
                  className={cn(
                    "px-3 py-2 rounded-full border",
                    (watch("preferredGenders") || []).includes(g)
            ...
            {errors.preferredGenders && (
              <p className="text-sm text-destructive">
                {errors.preferredGenders.message as any}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
              <Input
                ...
                onChange={(e) => {
                  ...
                  setValue("ageRange", [v, ageRange[1]]);
                }}
                className="w-24"
              />
              <span>to</span>
              <Input
                ...
                onChange={(e) => {
                  ...
                  setValue("ageRange", [ageRange[0], v]);
                }}
                className="w-24"
              />
            </div>
            {errors.ageRange && (
              <p className="text-sm text-destructive">
                {errors.ageRange.message as any}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="mt-2">
              <Slider
                ...
                onValueChange={(v) => {
                  const nv = [v[0], v[1]] as [number, number];
                  setTimeCommitment(nv);
                  setValue("timeCommitment", nv);
                }}
                min={0}
                max={80}
                step={1}
            ...
            {errors.timeCommitment && (
              <p className="text-sm text-destructive">
                {errors.timeCommitment.message as any}
              </p>
            )}
          </div>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {selected.map((s) => (
            <button
              ...
              type="button"
              onClick={() => toggle(s)}
              className="flex items-center gap-2 px-2 py-1 rounded-full border bg-primary text-primary-foreground text-xs"
            >
          ...
          <div className="absolute z-50 left-0 right-0 mt-2 bg-popover border rounded-md shadow-md max-h-56 overflow-y-auto p-2">
            <div className="px-2 pb-2 border-b border-border mb-2">
              <div className="text-xs text-muted-foreground">
                Tap an item to toggle selection
              </div>
            </div>
            {filtered.length === 0 ? (
              ...
              filtered.map((o) => (
                ...
                >
                  <span
                    className={cn(
                      "w-4 h-4 inline-flex items-center justify-center rounded-sm text-xs",
                      selected.includes(o)
                        ? "bg-primary text-primary-foreground"
